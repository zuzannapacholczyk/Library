...
public class BooksPanelModel {
	...
	private void setListElements() {
		...
		List<Department> departments = departmentOperations.getDepartments(view
				.getRights());
	...
	private void addAuthorBookRelation() {
		...
		if (model.getRowCount() > 0) {
			...
			int bookId = searchBookOperations.findBookItemByTitle(view
					.getTitleTextField().toLowerCase(), view.getRights());
			BookOperations bookOperations = new BookOperations();
			List<Integer> authorIds = new ArrayList<Integer>();
			for (int i = 0; i < model.getRowCount(); i++) {
				authorIds
						.add(Integer.parseInt((String) model.getValueAt(i, 0)));
			}
			if (bookOperations.addAuthorBookRelation(authorIds, bookId,
					view.getRights())) {
				ErrorDialog error = new ErrorDialog();
	...
	public void removeBook() {
		...
		BookOperations bookOperations = new BookOperations();
		if (view.getCopyIdTextField().length() == 0
				&& (view.getBookIdTextField().length() != 0 || book.getTitle()
						.length() != 0)) {
	...
	private void removeBook(Book book, BookOperations bookOperations) {
		if (view.getBookIdTextField().length() != 0) {
			...
			List<FoundBook> foundBooks = searchBookOperations.findBookByTitle(
					words, view.getRights());
			if (foundBooks.size() > 1) {
				ErrorDialog error = new ErrorDialog();
				error.showError("Ten sam tytul posiada kilka ksiazek. Musisz podac id");
			} else {
				if (bookOperations.removeBookByTitle(book.getTitle(),
						view.getRights())) {
					ErrorDialog error = new ErrorDialog();
					error.showSuccess("Ksiazka usunieta");
				} else {
	...
	public void addCopy() {
		...
		DepartmentOperations departmentOperations = new DepartmentOperations();
		int departmentId = departmentOperations.findDepartmentIdByName(view
				.getSelectedDepartment().toLowerCase(), view.getRights());
		...
		if (bookCopy.getBookId() == -1 && book.getTitle().length() > 0) {
			...
			int bookId = searchBookOperations.findBookItemByTitle(book
					.getTitle().toLowerCase(), view.getRights());
			if (bookId != -1) {
				...
				// add book
				if (bookOperations.addBook(book.getTitle(), view.getRights())) {
					bookId = searchBookOperations.findBookItemByTitle(book
							.getTitle().toLowerCase(), view.getRights());
					bookCopy.setBookId(bookId);
					if (bookOperations.addCopy(bookCopy, view.getRights())) {
						ErrorDialog error = new ErrorDialog();
